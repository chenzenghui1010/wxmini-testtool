<template>
  <div>
    ...
    <find-car-with-plate-number v-if="mapState.searchCarWithPlate" v-on:navigatetocar="navigateToCar"
                                v-bind:initcarno="carno" :region-id="regionId"></find-car-with-plate-number>
    ...
    <floor-list-control :floorlist="floorList" :currentName="currentFloorName" :selectfloorid="currentFloorIndex"
                        :locatefloorid="locateFloorIndex" v-on:onselect="onSelect"></floor-list-control>
              ...
              .then(data => {
                ...
                for (let i = 0; i < data.length; i++) {
                  ...
                  if (this.myMarker[i].mac === data[i].mac) {
                    ...
                    let zheng = new idrMarker({pos:this.statusList[i].pos, image:'./static/markericon/zhengchang.png'})
